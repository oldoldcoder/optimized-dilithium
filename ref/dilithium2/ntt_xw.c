...
int32_t pow_m(int32_t a, int32_t b) {
    int32_t res = 1;
    uint32_t y = b;
    while (y > 0) {
        if (y & 1) {
...
void compute_table() {
    g[0] = gi[0] = montgomery_reduce(MONT);
    for (int32_t i = 1; i < N_H; i++) {
        g[i] = montgomery_reduce(mul(g[i - 1], R_ROOT));
        gi[i] = montgomery_reduce(mul(gi[i - 1], R_R_INV));
    }
}
